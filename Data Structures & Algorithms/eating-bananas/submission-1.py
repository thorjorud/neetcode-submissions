class Solution:
    ...
    Time Complexity: O(n log(m))
        - We use binary search to find the correct rate k, which shrinks our 
        search space in half every step (log m). At each step of that search, 
        we have to loop through all n piles to calculate the total hours.
    Space Complexity: O(1)
        - We only use a few variables for our pointers and keeping track of 
